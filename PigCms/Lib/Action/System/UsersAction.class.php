...
class UsersAction extends BackAction{
            ...
            //根据表单提交的POST数据创建数据对象
                ...
                if($UserDB->save($_POST)){
					if($_POST['gid']!=$users['gid']){
						$fun=M('Function')->field('funname,gid,isserve')->where('`gid` <= '.$_POST['gid'].' AND `status` = 1')->select();
	...
	public function addfc(){
		...
		$gid=session('gid');
		$fun=M('Function')->field('funname,gid,isserve')->where('`gid` <= '.$gid.' AND `status` = 1')->select();
		foreach($fun as $key=>$vo){
			$queryname.=$vo['funname'].',';
		}
		$open['queryname']=rtrim($queryname,',');
