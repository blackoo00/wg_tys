...
>
<div class="cLineB">
  <h4>关于<font style="color:red;"><?php echo ($consultb['0']['consult']['title']); ?></font>的咨询详细</h4>
...
<div class="msgWrap">
  <form method="post"  action="index.php?ac=importtxt&amp;id=9379&amp;wxid=gh_858dwjkeww5" id="info" >
    ...
    <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
      <THEAD>
        <TR>
          <th><?php echo ($consult["doctor"]["name"]); ?></th>
          <th><?php echo ($consult["custom"]["name"]); ?></th>
        </TR>
      </THEAD>
      <TBODY>
        <TR></TR>
        <?php if(is_array($consultb)): $i = 0; $__LIST__ = $consultb;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
            <td style="line-height:22px; font-size:14px; letter-spacing:1px;"><?php if($list["dtalk"] == 1): ?><div><?php echo ($list["content"]); ?></div>
                <span style="float:right; color:rgba(68,221,221,0.8);"><?php echo (friendlydate($list["time"])); ?></span><?php endif; ?>
            </td>
            <td style="line-height:22px; font-size:14px; letter-spacing:1px;"><?php if($list["ctalk"] == 1): ?><div><?php echo ($list["content"]); ?></div>
                <span style="float:right; color:rgba(68,221,221,0.8);"><?php echo (friendlydate($list["time"])); ?></span><?php endif; ?>
            </td>
