...
<div class="msgWrap">
  <form method="post"  action="index.php?ac=importtxt&amp;id=9379&amp;wxid=gh_858dwjkeww5" id="info" >
    ...
    <style type="text/css">
      .doctor{display: inline-block; border-radius: 10px; border: 1px solid #DDD; cursor: pointer; font-size: 1.3em; margin: 0 8px 7px 0;}
      .doctor a{text-decoration: none; display: block; padding: 2px 22px;}
      .doctor:hover{background-color: #ECF5E5;}
      #stars span{display: inline-block; width: 17px; height: 17px; background-image: url(/tpl/static/images/star.gif);}
    </style>
<!--     <?php if(is_array($doctor)): $i = 0; $__LIST__ = $doctor;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><span class='doctor' <?php if(($list["id"]) == $did): ?>style="background-color: #ECF5E5;"<?php endif; ?>>
        <a href="<?php echo U('Comment/index',array('did'=>$list['id']));?>" title="<?php echo ($list["name"]); ?>"><?php echo ($list["name"]); ?></a>
    ...
    <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
      ...
      <TBODY>
        <TR></TR>
        <?php if(is_array($comment)): $i = 0; $__LIST__ = $comment;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
            <td><?php echo ($list["doctor"]["name"]); ?></td>
            <td><?php echo ($list["custom"]["name"]); ?></td>
            <td>
              <div id="stars">
                <?php $__FOR_START_608040266__=0;$__FOR_END_608040266__=$list["stars"];for($i=$__FOR_START_608040266__;$i < $__FOR_END_608040266__;$i+=1){ ?><span></span><?php } ?>
              </div>
            </td>
