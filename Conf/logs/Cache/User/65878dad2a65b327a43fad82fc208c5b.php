...
<div class="cLine">
  <div class="pageNavigator left">
    <a href='<?php echo U("Doctor/add");?>' title='新增医生' class='btnGrayS vm bigbtn'>
      <img src="<?php echo RES;?>/images/text.png" class="vm" />
      新增医生
    </a>
  </div>
...
<div class="msgWrap">
  <form method="post"  action="index.php?ac=importtxt&amp;id=9379&amp;wxid=gh_858dwjkeww5" id="info" >
    ...
    <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
      ...
      <TBODY>
        <TR></TR>
        <?php if(is_array($doctor)): $i = 0; $__LIST__ = $doctor;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
            <td><?php echo ($list["name"]); ?></td>
            <td><input id="hid" name="hid" type="hidden" value="<?php echo ($list["hospital"]["id"]); ?>"><?php echo ($list["hospital"]["name"]); ?></td>
            <td><?php echo ($list["persition"]); ?></td>
            <td><?php echo ($list["followers"]); ?></td>
            <td><img style="width:120px; height='auto';" src="<?php echo ($list["qrcode"]); ?>"></td>
            <!--  -->
			<td><?php echo ($list["orderNums"]); ?>（<a href="<?php echo U('Store/orders',array('did'=>$list['id']));?>" style="color:blue">明细</a>）</td>
            <td class="norightborder"> 
              <a href="<?php echo U('Doctor/details',array('id'=>$list['id']));?>" title="查看详细">查看详细</a>
              <a href="<?php echo U('Doctor/edit',array('id'=>$list['id']));?>" title="编辑文本">编辑</a>
              <a onclick="return confirm('确定要删除么？');" href="<?php echo U('Doctor/del',array('id'=>$list['id'],'hid'=>$list['hospital']['id']));?>">删除</a>
            </td>
