...
td{text-align: -webkit-left;font-size:12px;}
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		...
		<tr>
			<td align="right" height="62" width="">菜单类型：</td>
			<td>
				<div class="mr15 l">
				<select name="type">
					<option value="click" <?php if(($show["type"]) == "click"): ?>selected<?php endif; ?>>关键词推送</option>
					<option value="view" <?php if(($show["type"]) == "view"): ?>selected<?php endif; ?>>外链跳转</option>
					<option value="scancode_waitmsg" <?php if(($show["type"]) == "scancode_waitmsg"): ?>selected<?php endif; ?>>扫描带提示</option>
					<option value="scancode_push" <?php if(($show["type"]) == "scancode_push"): ?>selected<?php endif; ?>>扫码推事件</option>
					<option value="pic_sysphoto" <?php if(($show["type"]) == "pic_sysphoto"): ?>selected<?php endif; ?>>系统拍照发图</option>
					<option value="pic_photo_or_album" <?php if(($show["type"]) == "pic_photo_or_album"): ?>selected<?php endif; ?>>拍照或者相册发图</option>
					<option value="pic_weixin" <?php if(($show["type"]) == "pic_weixin"): ?>selected<?php endif; ?>>微信相册发图</option>
					<option value="location_select" <?php if(($show["type"]) == "location_select"): ?>selected<?php endif; ?>>发送位置</option>
				</select>
				</div>
				<div class="system l"></div>
			</td>
		...
		<tr bgcolor="#F1F1F1">
			...
			<td>
				<div class="mr15 l"><input id="menu_keyword" class="txt" name="keyword" title="关联关键词" value="<?php echo ($show["keyword"]); ?>" type="text"></div>
				<div class="system l"></div>
			</td>
		</tr>
		<tr>
			...
			<td>
				<div class="mr15 l"><input id="menu_key" class="txt" name="url" size="60" title="外链接url" value="<?php echo ($show["url"]); ?>" type="text"> <br/>非认证服务号或未开启用户授权请不要填写本站url  (<span style="color:red">微信自定义菜单已不支持一键拨号</span>)</div>
				<div class="system l"></div>
			</td>
		</tr>
		<tr bgcolor="#F1F1F1">
