...
<div class="msgWrap">
  <form class="form" method="post" action="<?php echo U('Hospital/upsave');?>" enctype="multipart/form-data" >
    <TABLE class="userinfoArea" style=" margin:20px 0 0 0;" border="0" cellSpacing="0" cellPadding="0" width="100%">
     <input type="hidden" name="id" value="<?php echo ($info["id"]); ?>" />
      <THEAD>
        <TR>
          <TH valign="top">
            <span class="red">*</span><label for="keyword">医院名称：</label>
          </TH>
          <TD>
		...
		<TR>
          <TH valign="top">
            <span class="red">*</span><label for="keyword">所属销售：</label>
          </TH>
          <TD>
            <input type="hidden" name="sid2" id="sid2" value="<?php echo ($info["hid"]); ?>">
            <select name="sid" id="sid" style="width:200px">
              <option value="">--请选择销售人员--</option>
              <?php if(is_array($sname)): $i = 0; $__LIST__ = $sname;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($list["id"]); ?>" <?php if(($list["id"]) == $info["sid"]): ?>selected<?php endif; ?> ><?php echo ($list["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </TD>
        </TR>
      </THEAD>
