...
<div class="cLine">
  <div style="float:right;line-height:30px;">
    <form action="#" method="post">
      <input type="text" placeholder="请输入患者姓名" value="<?php echo ($_POST['search']); ?>" class="px" name="search" />    
...
<div class="msgWrap">
  <form method="post"  action="index.php?ac=importtxt&amp;id=9379&amp;wxid=gh_858dwjkeww5" id="info" >
    ...
    <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
      ...
      <TBODY>
        <TR></TR>
        <?php if(is_array($custom)): $i = 0; $__LIST__ = $custom;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
            <td><?php echo ($list["name"]); ?></td>
            <td><?php if(($list["sex"]) == "1"): ?>男<?php else: ?>女<?php endif; ?></td>
            <td><?php echo ($list["doctor"]["name"]); ?></td>
            <td><?php echo ($list["tel"]); ?></td>
			      <td><?php echo ($list["orderNums"]); ?>（<a href="<?php echo U('Store/orders',array('cid'=>$list['id']));?>" style="color:blue">明细</a>）</td>
            <td class="norightborder">
               <a href="<?php echo U('Custom/details',array('id'=>$list['id']));?>" title="详细信息">详细信息</a>
               <!-- <a href="<?php echo U('Consult/index',array('id'=>$list['id']));?>" title="咨询信息">咨询信息(<?php echo ($list["consultnum"]); ?>)</a> -->
            </td>
          </tr>
