...
<body class="index">
    <div class="ol">
<form action="<?php echo U('Steward/bsuger',array('token'=>$token,'wecha_id'=>$wecha_id));?>" data-ajax="true" data-ajax-begin="Begin" data-ajax-method="Post" data-ajax-success="OnSuccess" id="form0" method="post">    
    ...
    <script>
        $(function(){
            $("#loadmore").bind("click",function(event){
                ...
                $.ajax({
                    url:"<?php echo U('Steward/loadmorebsuger',array('token'=>$token,'wecha_id'=>$wecha_id));?>",
                    data:{trnums:trnums},
                    success:function(data){
                        if(data=="error"){
                            alert("没有更多数据");
