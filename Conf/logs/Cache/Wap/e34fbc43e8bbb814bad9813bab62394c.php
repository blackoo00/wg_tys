...
<?php if(($product["price"]) != $product["vprice"]): ?><span class="okp">会员价：<em id="vprice">￥<?php echo ($product["vprice"]); ?></em></span><?php endif; ?>
<span>原价：<i>￥<?php echo ($product["oprice"]); ?></i></span>
</div>
...
<div class="i-row act">
<a href="javascript:;" onClick="Collection()" class="collection">加入收藏</a>
...
<div style="background:#4B514F;position:fixed;right:10px;bottom:40px;border-radius:5px;padding:5px 10px;background-color:rgba(0,0,0,0.7);font-size:16px;z-index:9999">
...
$(document).ready(function() {
	$(".com_pg-num").click(function(){
		...
		$.get("<?php echo U('Store/getcomment',array('token'=>$token,'mid'=>$my['id'],'pid'=>$product['id'],'wecha_id'=>$_GET['wecha_id']));?>" + '&page='+page, function(response){
...
function add_cart() {
	...
	$.ajax({
		url: "<?php echo U('Store/addProductToCart',array('token'=>$token,'mid'=>$my['id'],'id'=>$product['id'],'wecha_id'=>$_GET['wecha_id']));?>" + '&count='+count + '&did=' + did,
...
function QuickBuy() {
	...
	$.ajax({
		...
		success: function(data) {
			if(data){
				location.href = "<?php echo U('Store/cart',array('token' => $token,'mid'=>$my['id']));?>";;
			} else {
				return floatNotify.simple('抱歉，您的请求不正确');
			}
		}
...
function Collection() {
	$.ajax({
		url: "<?php echo U('Store/addCollection',array('token'=>$token,'mid'=>$my['id'],'id'=>$product['id'],'wecha_id'=>$_GET['wecha_id']));?>",
		success: function(data) {
			if(data){
				if(data==1){
...
window.shareData = {  
            ...
            "timeLineLink": "<?php echo C('site_url') . U('Store/product',array('token' => $_GET['token'],'id'=>$product['id'],'mid'=>$my['id']));?>",
            "sendFriendLink": "<?php echo C('site_url') . U('Store/product',array('token' => $_GET['token'],'id'=>$product['id'],'mid'=>$my['id']));?>",
            "weiboLink": "<?php echo C('site_url') . U('Store/product',array('token' => $_GET['token'],'id'=>$product['id'],'mid'=>$my['id']));?>",
