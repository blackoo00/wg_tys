...
class Log {
    ...
    static function write($message,$level=self::ERR,$type='',$destination='',$extra='') {
        ...
        if(self::FILE == $type) { // 文件方式记录日志
            if(empty($destination))
                if($level==self::ERR||$level==self::CRIT||$level==self::ALERT||$level==self::EMERG){
					$destination = LOG_PATH.date('y_m_d').'_error.log';
				}else if($level==self::DEBUG){
					$destination = LOG_PATH.date('y_m_d').'_debug.log';
				}else{
					$destination = LOG_PATH.date('y_m_d').'.log';
				}
            //检测日志文件大小，超过配置大小则备份日志文件重新生成
